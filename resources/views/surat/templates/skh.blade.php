...
@php
    ...
    $kodeMap = ['ktp' => 'SKHK', 'kk' => 'SKHKK', 'akte' => 'SKHA'];
...
<div class="page">
    ...
    <div class="indent">
        <table style="margin-left:20px;">
            ...
            <tr><td>No KTP</td><td>: {{ $data['nik'] ?? $user->nik ?? '-' }}</td></tr>
        </table>
    </div>

    <p class="mt-2 indent">
        Adalah benar nama tersebut di atas adalah warga yang berdomisili di Dusun {{ $data['dusun'] ?? optional($profile)->dusun ?? '-' }}, Desa Limapoccoe, Kecamatan Cenrana, Kabupaten Maros,
        yang telah kehilangan dokumen <strong>{{ strtoupper($data['jenis_dokumen'] ?? '-') }}</strong> dengan nomor <strong>{{ $data['no_dokumen'] ?? '-' }}</strong>.
        Dokumen tersebut diperkirakan hilang/tercecer di <strong>{{ $data['perkiraan_lokasi_hilang'] ?? '-' }}</strong>.
    </p>

    <p class="indent">Demikian surat keterangan ini kami buat dengan sebenarnya untuk digunakan seperlunya.</p>
